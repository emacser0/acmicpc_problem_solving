...
int MinDistance(std::vector<int>& distance)
{
	int min = 10e10;
...
int main()
{
	...
	std::vector<std::vector<IntPair>> graph(V, std::vector<IntPair>());
	...
	for (int i = 0; i < E; ++i)
	{
		...
		graph[u - 1].push_back(IntPair(v - 1, w));
		graph[v - 1].push_back(IntPair(u - 1, w));
	}

	std::vector<int> distance(V, 10e10);
